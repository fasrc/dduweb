...
def userIsInGroup(username, groupname):
	"""return boolean of whether or not the given user is in the given group

	This will raise an Exception if there are any issues (e.g. the group doesn't exist).
	This does not test if the user exists, though.
	"""
	if username in grp.getgrnam(groupname).gr_mem: return True
	return False
